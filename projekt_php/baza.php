...
if(isset($_POST['Akceptuj'])) {
          $id = mysqli_real_escape_string($db, $_POST['Akceptuj']);
          $query = "INSERT INTO dodane (wykonawca,tytul,email) SELECT wykonawca,tytul,email FROM dodaj WHERE id = $id";
          $query2 = "DELETE FROM dodaj WHERE id = $id";
        ...
        if(isset($_POST['Odrzuc'])) {
          ...
          $query = "DELETE FROM dodaj WHERE id = $id";
          mysqli_query($db, $query);
          header("location: baza.php");
        }
...
<body>
  ...
  <?php
    $request = "SELECT * FROM dodaj";
    $result =  mysqli_query($db,$request);
    $num = mysqli_num_rows($result);
   if($num!=0) echo"<table class='table table-dark'><th>WYKONAWCA</th><th>TYTUŁ</th><th>EMAIL</th><th>Akceptacja</th>";
    if($num==0){echo "<p class= dodaaaj>Brak piosenek do akceptacji <a href=wszystkie_piosenki.php class='link-danger'>Kliknij i sprawdź wszystkie utwory</a><p>";}
      else{
    while($row = mysqli_fetch_array($result)){
      ...
      <form method='post'>
      <button class='btn btn-success btn-sm' type='submit' name=Akceptuj id='Akceptuj' value='$id'>Akceptuj</button>
      <button class='btn btn-danger btn-sm' type='submit' name=Odrzuc id='Odrzuc' value='$id'>Odrzuć</button>
      </form>
